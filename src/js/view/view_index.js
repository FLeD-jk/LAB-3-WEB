...
export default class PostView {
  ...
  renderPost(title, text, tagsHTML, timestamp) {
    ...
    const truncatedText = text.length > 150 ? text.substring(0, 230) + '...' : text

    const postHTML = `
    <a class="Card-discuss card border-0" href="./discuss?title=${encodeURIComponent(title)}&time=${encodeURIComponent(timestamp)}&content=${encodeURIComponent(text)}" style="text-decoration: none; color: inherit; user-select: none;">
